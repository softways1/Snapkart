...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($entered_otp == $_SESSION['otp']) {
        ...
        $insert = mysqli_query($conn, "INSERT INTO customers (name, email, password, address, pincode) 
            VALUES ('$name', '$email', '$password', '$address', '$pincode')");

        if ($insert) {
            unset($_SESSION['otp']);
            unset($_SESSION['temp_customer']);
            $show_success = true;
        } else {
...
<div class="bg-gradient-to-r from-orange-100 via-pink-300 to-purple-200 flex justify-center items-start pt-25 h-screen">
    <?php if ($show_success): ?>
        <div class="flex flex-col items-center bg-white rounded-xl shadow-lg p-8 w-[400px] bg-gradient-to-r from-orange-300 via-pink-300 to-blue-300">
            ...
            <h2 class="text-3xl font-semibold bg-gradient-to-r  from-orange-400 via-pink-500 to-blue-500 text-transparent bg-clip-text mb-2">Registration Successful!</h2>
